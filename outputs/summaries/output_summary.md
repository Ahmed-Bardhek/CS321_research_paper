# Output Summary

## Files
- `outputs/model_outputs/gpt_model_outputs.json`
- `outputs/model_outputs/llama_model_outputs.json`
- `outputs/model_outputs/qwen_mistral_model_outputs.json`
- `outputs/model_outputs/combined_model_outputs.json`
- `outputs/model_outputs/combined_model_outputs.csv`
- `outputs/logs/experiment_log.txt`
- `outputs/logs/run_metadata.json`
- `outputs/summaries/run_summary.json`

## Aggregate Accuracy from Generated Output Samples

| Model | Accuracy | Average quality | Average tokens | Average response time |
|---|---:|---:|---:|---:|
| GPT model | 0.850 | 4.85 | 128.9 | 2.06 |
| Llama model | 0.700 | 3.98 | 128.9 | 2.67 |
| Qwen/Mistral model | 0.787 | 4.08 | 128.9 | 2.42 |

## Important Note

These files are  s based on the paper's current  results. They should be replaced by real model outputs after the experiment is executed.